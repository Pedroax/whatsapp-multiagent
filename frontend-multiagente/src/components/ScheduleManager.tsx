...
export const ScheduleManager: React.FC<ScheduleManagerProps> = ({ empresaId }) => {
  ...
  const carregarAgendamentos = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8000"}/api/ia-control/agendamentos/${empresaId}`);
  ...
  const salvarAgendamento = async () => {
    try {
      const url = agendamentoEditando?.id
        ? `${import.meta.env.VITE_API_URL || "http://localhost:8000"}/api/ia-control/agendamentos/${agendamentoEditando.id}`
        : '${import.meta.env.VITE_API_URL || "http://localhost:8000"}/api/ia-control/agendamentos';
  ...
  const deletarAgendamento = async (id: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8000"}/api/ia-control/agendamentos/${id}`, {
        method: 'DELETE'
      });
