...
export const ApprovalQueue: React.FC<ApprovalQueueProps> = ({ empresaId }) => {
  ...
  const carregarFila = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/ia-control/fila-aprovacao/${empresaId}`);
  ...
  const aprovarMensagem = async (mensagemId: string, textoFinal?: string) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/ia-control/aprovar-mensagem', {
        method: 'POST',
  ...
  const recusarMensagem = async (mensagemId: string) => {
    setProcessando(mensagemId);
    try {
      const response = await fetch('http://localhost:8000/api/ia-control/recusar-mensagem', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          mensagem_id: mensagemId,
